...
class GridWorld:
    ...
    def step(self, action):
        ...
        if not self._within_bounds(self.current_pos):
            ...
            cost += 2 # damage cost, assuming physical wall collision

        #reward += self._incremental_reward()
        reward = -1
        if self._in_charging_location():
            #cost += self.max_battery / 10 # time taken to charge battery
            reward -= 10
            cost = self.current_battery-self.max_battery
            self.current_battery = self.max_battery
        else:
            cost += 1 # fuel cost
        ...
        if self.carrying_object and self._reached_target():
            ...
            self.carrying_object = False
            reward += 10 # extra reward for delivery
            self.delivery_location = self._random_point()
    ...
    def reset(self):
        self.__init__(self.max_battery)
